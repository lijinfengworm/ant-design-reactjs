 ...
 * 内部表格组件
 */
class InnerQueryMachine extends React.PureComponent {
  ...
  render() {
    ...
    var queryTable = this.state.data.map(function(cad){
    	console.info(cad);
    	let onText, runningText, onIcon, runningIcon, leftStatus;

    	if(cad.status.power_status == 1){
    		onText = '开';
    		onIcon = 'right-circle';
    	}else{
    		onText = '关';
    	...
    	if(cad.status.machine_status == 0){
    		runningText = '宕机';
    		runningIcon = 'cloud-download';
    	}else{
    		runningText = '运行中';
    		runningIcon = 'cloud';
    	}
...
export default InnerQueryMachine;
